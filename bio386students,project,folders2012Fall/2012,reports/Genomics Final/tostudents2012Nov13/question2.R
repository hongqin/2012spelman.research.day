...
list.files()
list.files(, pattern="GSE3821")

#gene expression CV table
CV.tb = read.csv("JessNCBI_log2CV.csv")  #CV table
...
summary(lm(RLS.tb2$RLS_Del_alpha ~ RLS.tb2$myStddev))  #p=0.9373
summary(lm(RLS.tb2$RLS_Del_alpha ~ RLS.tb2$myMean)) #p=0.316
...
m=lm(fit.tb2$YPD ~ fit.tb2$myCV); summary(m) #p=2.2E-16 #strong positive correlation
...
summary(lm(RLS_Del_alpha ~ pDegree, data=RLS.tb2)) #p=0.2687
summary(lm(RLS_Del_alpha ~ plusGDegree, data=RLS.tb2))#p=0.0759
summary(lm(RLS_Del_alpha ~ minusGDegree, data=RLS.tb2)) #p=0.413
summary(lm(RLS_Del_alpha ~ myCV, data=RLS.tb2)) #p=0.123
summary(lm(RLS_Del_alpha ~ myCV + pDegree + plusGDegree + minusGDegree, data=RLS.tb2)) #p=0.2882
summary(lm( myCV ~ RLS_Del_alpha + pDegree + plusGDegree + minusGDegree, data=RLS.tb2)) #p=0.2146
summary(lm( RLS_Del_alpha ~ myCV + pDegree + YPE + + plusGDegree + minusGDegree, data=RLS.tb2)) #p=0.379  
summary(lm( RLS_Del_alpha ~ YPD, data=RLS.tb2)) #p=0.988  
summary(lm( RLS_Del_alpha ~ YPG, data=RLS.tb2)) #p=0.1693  
summary(lm( RLS_Del_alpha ~ YPE, data=RLS.tb2)) #p=0.05846  
summary(lm( RLS_Del_alpha ~ myCV  + YPD + YPE + YPG, data=RLS.tb2)) #p=0.06  
summary(lm( myCV ~ RLS_Del_alpha  + YPD + YPE + YPG, data=RLS.tb2)) #p=0.1834  
